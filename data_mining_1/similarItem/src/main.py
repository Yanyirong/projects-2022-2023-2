...
for i in range(0, numDocs):
  ...
  for i in range(0,len(words)-2):
    # form a shingle
    hashxxx=sha1()#sha1算法，hashlib很多加密算法


    shingle = words[i] + " " + words[i+1] + " " + words[i+2]
    hashxxx.update(shingle.encode('utf-8'))
    
    shinglesInDoc.add(int.from_bytes(hashxxx.digest() ,byteorder='little')& 0xffffffff)
  ##### end of TODO #####
...
elems_num = 499500
# sample (fake) 
JSim = [0 for i in range(elems_num)]
estJSim = [0 for i in range(elems_num)]
def getTriangleIndex(i, j):
  # i == j  is an illegal situation
  if i == j:
    pass
  #  j < i then swap the values
  if j < i:
    tmp = j
    j = i
    i = tmp
  k = int((numDocs-(i + 1)/ 2) * i - i + j) - 1
  return k
### end of TODO ###
...
for docID in docNames:
  ...
  for i in range (0,10):
    minhash_number = nextPrime + 1
    for index in shingleIDSet:
      hash_number = (a_list[i] * index + b_list[i]) % nextPrime
      if hash_number < minhash_number:
        minhash_number = hash_number
    signature.append(minhash_number)
...
for i in range(0, numDocs):
  ...
  for j in range(i + 1, numDocs):
    ...
    for k in range(0,10):
      something = something + (signature1[k]==signature2[k])
    something = something / numHashes

    estJSim[getTriangleIndex(i, j)] = something
...
for i in range(0, numDocs):  
  for j in range(i + 1, numDocs):
    ...
    if estJ > threshold:
      ...
      J = len(intersection_set)/len(union_set)
      ### end of TODO ###
