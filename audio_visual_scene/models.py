...
class EarlyConcatDense(nn.Module):
...
class LateConcatDense(nn.Module):
    ...
    def forward(self, audio_feat, video_feat):
        ...
        video_output = self.classifier(video_emb)
        audio_output = self.classifier(audio_emb)
        output = (video_output + audio_output)/2
        return output